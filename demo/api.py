...
_STATIONS = [(0.5, 0.32), (0.2, 0.66), (0.8, 0.66)]   # hull station-keeping in the formation box
...
def _destroyer_ui_shape(rich: dict) -> dict:
    ...
    # Per-hull model + signed-delta come from the REAL sealed fleet record (build_fleet_state),
    # not hardcoded fixtures — they reflect the last sealed fleet-merge (regenerated by demo_up.sh).
    fleet_ships = shore_rich.get("accepted_deltas") or []
    ...
    for i, sh in enumerate(ships):
        ...
        fs = fleet_ships[i] if i < len(fleet_ships) else None
        local_rmse = (fs or {}).get("local_train_rmse") or round(incumbent_rmse * (1.0 + 0.012 * i), 6)
        n_samples = (fs or {}).get("n_samples") or 240
        status = (fs or {}).get("status") or "pending"     # in this hull's last delta: merged vs pending
        delta = round(merged_rmse - local_rmse, 6)         # signed Δ this hull contributed toward the merge
        destroyers.append({
            ...
            "subsystems": subs,
            "model": {"version": 7 - min(i, 1), "local_rmse": local_rmse, "n_samples": n_samples},
            "sync": {"delta": delta, "signed": True, "attested": True, "status": status},
        })
