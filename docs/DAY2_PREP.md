...
| 2 | **Two different "honest" NV063 numbers on disk** — `eval/RESULTS.md` headlines **0.36**; `curated_metrics.json`/ROADMAP headline **0.57** (and 0.57 sits *inside* 0.36's CI of 0.18–0.57, so not even a distinguishable improvement). A judge who opens both files concludes you can't keep your own scoreboard straight. **VERIFIED.** | CRIT | Re-run `eval/score.py` tonight, pick ONE number + n + CI, overwrite every doc + deck, archive the loser. | Narrative, 1hr |
