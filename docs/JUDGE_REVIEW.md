...
- **The UUV C2-link anomaly detector (Framing B — REAL now):** a River HalfSpaceTrees detector (`serve/receiver/`) on **genuine UUV command-link data** (`serve/receiver/data/uuv1-c2-anom.json` — failed-integrity, 20s latency spikes, ABORT_MISSION) — **precision@25 0.96 / F1 0.83**, the live **Node-3 MLflow** (:5050) registers it as `uuv1_anomaly_deploy`, and the edge receiver pulls it. The Node-1/2 onboard brain. **Reproduce:** `bash deploy/mlflow/run.sh` then `MLFLOW_TRACKING_URI=http://localhost:5050 python3 serve/receiver/train_c2.py`.
- **In-flight (the team's last-day lanes):** Claire's **sequence-autoencoder** on UUV telemetry (registers as `theseus-uuv`); the 2 Raspberry Pis as live UUV nodes. *The flywheel + the C2 detector run today on Node 3; these make it the full hardware loop.*
...
- **Framing B — the platform's OWN UUV systems** (the anchor): onboard health / C2 anomaly. **Now backed by REAL UUV data** — the C2-link detector above runs on genuine UUV command-link traffic (`uuv1-c2-anom.json`). The earlier machinery datasets (gas turbine / turbofan) were **proxies** and are labeled as such; a complementary subsystem-telemetry model (BlueROV2/glider) is in-flight. *We do not call a jet engine a UUV.*
